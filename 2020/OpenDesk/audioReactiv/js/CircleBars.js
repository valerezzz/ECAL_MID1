...
CircleBars.prototype = {
  ...
  draw: function(data) {
    ...
    if (this.rotation) this.ctx.rotate(this.angle);
    ...
    for (var i = 0; i < 360; i++) {
      ...
      var x2 = 0 + Math.cos(i * Math.PI / 180) * (this.radius + 600);
      var y2 = 0 + Math.sin(i * Math.PI / 180) * (this.radius + 600);
      this.ctx.beginPath();
    ...
    for (var i = 0; i < 360; i++) {
      ...
      // calculate second point with the same orientation (angle)
      var x2 = 0 +
          Math.cos(i * Math.PI / 180) * (this.radius + 1200 - data[i + 10] * 7);
      var y2 = 0 +
          Math.sin(i * Math.PI / 180) * (this.radius + 1200 - data[i + 10] * 7);
      this.ctx.beginPath();
      this.ctx.moveTo(x1, y1);
      this.ctx.lineTo(x2, y2);
      this.ctx.stroke();
